...
public class EndUser extends Node implements Runnable {
	...
	EndUser(Terminal terminal, int port) {
		try {
	...
	public void run() {
		while (true) {
			...
			sendMessage(input, CONSTANT.ROUTER_1);
		}
	}
	
	 public static void main(String[] args) {
	    	try {
	    		Terminal terminal = new Terminal("EndUser 1");
	    		(new EndUser(terminal, CONSTANT.END_USER_1)).run();
