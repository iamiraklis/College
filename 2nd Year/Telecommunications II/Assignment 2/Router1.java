...
public class Router1 extends Node implements Runnable {
	...
	Router1(Terminal terminal, int port) {
		try {
	...
	public synchronized void onReceipt(DatagramPacket packet) {
		...
		if (packetPort == CONSTANT.CONTROLLERPORT && content.contains("forwardMessage")) {
			sendMessage(message, port);
		}
		if (packetPort == CONSTANT.END_USER_1) {
			message = content;
			sendMessage("flowtable", CONSTANT.CONTROLLERPORT);
		}
		if (packetPort != CONSTANT.CONTROLLERPORT && packetPort != CONSTANT.END_USER_1) {
	...
	public static void main(String[] args) {
		try {
			...
			(new Router1(terminal, CONSTANT.ROUTER_1)).run();
		} catch (java.lang.Exception e) {
			e.printStackTrace();
		}
	}
