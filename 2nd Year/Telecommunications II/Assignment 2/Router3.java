...
public class Router3 extends Node implements Runnable {
	...
	Router3(Terminal terminal, int port) {
		try {
	...
	public synchronized void onReceipt(DatagramPacket packet) {
		...
		if (packetPort == CONSTANT.END_USER_2) {
			message = content;
			sendMessage("flowtable", CONSTANT.CONTROLLERPORT);
		}
		if (packetPort != CONSTANT.CONTROLLERPORT && packetPort != CONSTANT.END_USER_2) {
	...
	public static void main(String[] args) {
		try {
			...
			(new Router3(terminal, CONSTANT.ROUTER_3)).run();
		} catch (java.lang.Exception e) {
			e.printStackTrace();
		}
	}
